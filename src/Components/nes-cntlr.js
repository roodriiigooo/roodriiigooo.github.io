...
//   }
  ...
  setVirtualCntlrPos() {
    if (this.dom.dpad && this.dom.btns) {
      ...
      // console.log("dpadRect", dpadRect);
      // console.log("btnsRect", btnsRect);

      this.current.dpad.top = dpadRect.y + document.documentElement.scrollTop;
      this.current.dpad.left = dpadRect.x;
      this.current.dpad.right = dpadRect.right;
      // console.log("D_PAD POSITION", this.current.dpad);

      this.current.btns.top = btnsRect.y + document.documentElement.scrollTop;
      this.current.btns.left = btnsRect.x;
      this.current.btns.right = btnsRect.right;
      // console.log("BUTTOM_POSITION", this.current.btns);
  ...
  dpadMove(e) {
    ...
    let centerX = (this.current.dpad.left) + 85;
    let centerY = (this.current.dpad.top) + 85;
